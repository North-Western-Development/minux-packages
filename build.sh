...
defaulttarget='riscv64-linux-musl'
...
rm -rf "$_TMP/minux-sysroot-$_TARGET"*
...
depcheck() {
    for dep in "$_TARGET-cc" ldid dpkg-deb fakeroot automake autoreconf m4 yacc ctags tar gzip bzip2 xz zstd ninja sed pgrep meson cmake curl git pkg-config perl; do
    ...
    export _DPKGARCH='musl-linux-riscv64'

    minuxpath="$(command -v "$_TARGET-cc")"
    sdk="${minuxpath%/*}/../share/minux/sysroot"
    minuxpath="${minuxpath%/*}/../share/minux/bin"
    PATH="$minuxpath:$PATH"

    export _MAKE _CPU _DPKGARCH _MACVER _MACVERNUM PATH
}
...
includedeps() {
    if [ -z "$dryrun" ]; then
        if [ -d "$sdk" ]; then
            export _SDK="$_TMP/minux-sysroot-$_TARGET"
            mkdir -p "$_SDK"
            cp -a "$sdk"/* "$_SDK"
        else
            error "sysroot not found"
        fi
    fi
...
main() {
    case "$1" in
        ...
        cleanall)
            rm -rf "$pkgdir"/*/pkg "$pkgdir"/*/src "$pkgdir"/*/*.deb "$bsroot"/debs/*.deb "$_TMP"/minux-sysroot-* "$bsroot/.builtpkgs"
        ;;
